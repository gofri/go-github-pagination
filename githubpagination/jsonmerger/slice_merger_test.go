...
func MergeInto(merger jsonmerger.JSONMerger, v any) error {
	return json.NewDecoder(merger.Merged()).Decode(v)
}

func _TestMultipleSlices(t *testing.T, merger jsonmerger.JSONMerger) {
	var result []int
	inputSlices := [][]int{
		{1, 2, 3},
		{4, 5, 6},
...
func TestMultipleSlices(t *testing.T) {
	_TestMultipleSlices(t, jsonmerger.NewUnprocessedSlice())
}
