...
func NewAsync[DataType any](onNext OnNextResponseSlice[DataType]) *Async[DataType] {
	adapter := func(response *http.Response, result *gh_search_result.Typed[DataType]) error {
...
type OnNextResponse[DataType any] func(*http.Response, *gh_search_result.Typed[DataType]) error
...
func (a *Async[DataType]) Paginate(requestFn any, args ...any) error {
	...
	isSearch := respType == gh_search_result.ResponseDataTypeSearch
...
func (a *Async[DataType]) HandlePage(data *gh_search_result.Typed[DataType], response *http.Response) error {
	if a.OnNext != nil {
...
func (a *Async[DataType]) getValidatedResponseDataType(requestFn any) (gh_search_result.ResponseDataType, error) {
	respDataType := gh_search_result.ResponseDataTypeUnknown
	...
	respDataType = gh_search_result.GetResponseDataType[DataType](fnType.Out(returnIndexData))
	if respDataType == gh_search_result.ResponseDataTypeUnknown {
...
func (a *Async[DataType]) withAsyncCtx(requestFn any, isSearch bool, args ...any) []reflect.Value {
	...
	ctx := WithOverrideConfig(args[argsIndexContext].(context.Context),
		WithDriver(pagination_drivers.NewGithubAsyncPaginationDriver(a, isSearch)),
