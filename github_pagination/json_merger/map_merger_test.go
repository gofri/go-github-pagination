...
func TestMultipleMaps(t *testing.T) {
	_TestMultipleMaps(t, json_merger.NewGitHubUnprocessedMap())
}

func _TestMultipleMaps(t *testing.T, merger json_merger.JsonMerger) {
	t.Parallel()
	...
	t.Run("test github map merger", func(t *testing.T) {
		...
		merger := json_merger.NewGitHubUnprocessedMap()
